...
void USART_PeripheralControl(USART_RegDef_t *pUSARTx, uint8_t EnorDi)
{
	if(EnorDi == ENABLE)
	{
		pUSARTx->CR1 |= (1 << USART_CR1_UE); //enable
	}
	else
	{
		pUSARTx->CR1 &= ~(1 << USART_CR1_UE); //disable

}
uint8_t USART_GetFlagStatus(USART_RegDef_t *pUSARTx , uint32_t FlagName)
{
 	if(pUSARTx->ISR & FlagName) // flag name actually gives masking info, see spi_driver.h @SPI_FLAG
	{
		return FLAG_SET;
	}
